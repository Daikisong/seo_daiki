...
DATABASE_URL='postgresql://postgres@127.0.0.1:55432/postgres?schema=public' pnpm db:admin -- set-index-status art-en-review-ugreen-pending index
```
...
- Admin publish gate smoke: forcing `art-en-review-ugreen-pending` to `index` exited 1, reported blocker issues, and wrote an `AuditLog` row with action `publish_gate_blocked`
