...
export function validateArticleContent(articles: Article[]) {
  for (const article of articles) {
    ...
    if (article.indexStatus === "index" && !isIndexableLocale(article.locale)) {
      throw new Error(
        `${article.id} uses planned locale ${article.locale}. Mark it noindex until the locale opens.`,
      );
    }
    assertPresent(article.title, `${article.id}.title`);
    assertPresent(
      article.affiliateDisclosure,
      `${article.id}.affiliateDisclosure`,
    );
    ...
    assertPresent(
      ...
      `${article.id}.expertCopy.comparisonIntro`,
    );
    assertPresent(
      article.expertCopy.inDepthHeading,
      `${article.id}.expertCopy.inDepthHeading`,
    );
    assertPresent(
      article.expertCopy.topThreeHeading,
      `${article.id}.expertCopy.topThreeHeading`,
    );
    assertFilledList(
      article.expertCopy.finalThoughts,
      `${article.id}.expertCopy.finalThoughts`,
    );
    assertFilledList(
      article.expertCopy.buyingChecklist,
      `${article.id}.expertCopy.buyingChecklist`,
    );
    assertFilledList(
      article.expertCopy.updateLog,
      `${article.id}.expertCopy.updateLog`,
    );
    assertFilledList(article.evidenceIds, `${article.id}.evidenceIds`);
    assertKnownArticleEvidenceIds(
      article.evidenceIds,
      `${article.id}.evidenceIds`,
    );
    if (article.trendSignalBox) {
      assertTrendSignalBox(
        article.trendSignalBox,
        `${article.id}.trendSignalBox`,
      );
    }
    if (article.marketplaceRule) {
      assertMarketplaceRule(
        article.marketplaceRule,
        `${article.id}.marketplaceRule`,
      );
    }
    if (article.requiresCountryBuyingRoutes) {
      assertFilledArticlePairs(
        article.countryBuyingRoutes,
        `${article.id}.countryBuyingRoutes`,
        "market",
        "route",
      );
    }
    ...
    if (article.sections.length < 3) {
      throw new Error(
        `${article.id}.sections must include issue bridge, buying checks, and review-signal guidance.`,
      );
    }
    assertQuickAnswerSection(
      article.sections[0],
    ...
    for (const section of article.sections) {
      ...
      assertKnownArticleEvidenceIds(
        section.evidenceIds ?? [],
        `${article.id}.sections.${section.heading}.evidenceIds`,
      );
    }
...
export function validateProductContent(products: Product[]) {
  for (const product of products) {
    ...
    assertPresent(
      ...
      `${product.id}.marketplaceSourceLabel`,
    );
    assertPresent(product.priceCheckedAt, `${product.id}.priceCheckedAt`);
    assertPresent(product.priceState, `${product.id}.priceState`);
    ...
    if (product.category === "portable air conditioner heatwave cooling") {
      ...
      assertPresent(
        firstPrice?.country,
        `${product.id}.priceSnapshots.country`,
      );
      assertPresent(
        firstPrice?.currency,
        `${product.id}.priceSnapshots.currency`,
      );
      assertPresent(
        firstPrice?.priceLabel,
        `${product.id}.priceSnapshots.priceLabel`,
      );
      assertPresent(
        firstPrice?.priceState,
        `${product.id}.priceSnapshots.priceState`,
      );
...
function assertMarketplaceRule(
  value: Article["marketplaceRule"],
  fieldName: string,
) {
  if (!value) {
...
function assertQuickAnswerSection(
  value: Article["sections"][number] | undefined,
  fieldName: string,
  ...
  if (
    !hasQuickAnswerRole &&
    (!isEnglishLocale || normalized(value.heading) !== "quick answer")
  ) {
    throw new Error(
      `${fieldName} must be marked role: "quick-answer" so localized headings can stay in the page language.`,
    );
  }
  const body = normalized(value.body);
  const englishBridgeTerms = [
    "buy",
    "compare",
    "check",
    "product",
  ...
  if (
    isEnglishLocale &&
    !englishBridgeTerms.some((term) => body.includes(term))
  ) {
    throw new Error(
      `${fieldName}.body must bridge the trend issue to a buyer decision.`,
    );
  }
  if (!isEnglishLocale && value.body.trim().length < 80) {
    throw new Error(
      `${fieldName}.body must contain a localized issue-to-product bridge.`,
    );
  }
}
...
function assertKnownArticleEvidenceIds(values: string[], fieldName: string) {
  for (const value of values) {
    ...
    if (!source) {
      throw new Error(
        `${fieldName} contains unknown article evidence id "${value}". Add a source record before publishing.`,
      );
    }
...
function assertPresent(value: string | undefined, fieldName: string) {
  if (!value || value.trim().length === 0) {
    throw new Error(
      `${fieldName} is required. Generate it in the content layer; do not synthesize it in React.`,
    );
  }
}
...
function assertFilledArticlePairs<T>(
  ...
  for (const value of values) {
    ...
    assertPresent(
      ...
      `${fieldName}.${String(firstKey)}`,
    );
    assertPresent(
      typeof secondValue === "string" ? secondValue : undefined,
      `${fieldName}.${String(secondKey)}`,
    );
  }
}
