...
export function buildQualityGateReport(result: QualityGateResult): PipelineQualityGateResult {
  const blockers = allQualityGateBlockers(result);
  const status = qualityGateReportStatus(result, blockers);

  return {
    status,
    canPublish: status === "PASS" && result.canPublishIndexable,
...
export function repairTasksFromQualityGateReport(report: PipelineQualityGateResult): RepairTask[] {
  return report.blockers.map((blocker, index) => ({
    id: `quality-gate-${index + 1}`,
    source: "quality-gate",
    code: blocker.code,
    severity: blocker.severity === "hard" || blocker.severity === "hold" ? "blocked" : "repair",
    target: blocker.target,
...
function allQualityGateBlockers(result: QualityGateResult): QualityGateBlocker[] {
  return [
...
function qualityGateReportStatus(result: QualityGateResult, blockers: QualityGateBlocker[]): PipelineQualityGateResult["status"] {
  if (result.status === "PASS" && blockers.length === 0) {
    return "PASS";
  }
  if (result.hardGate.blockers.length > 0 || result.hreflangGate.blockers.length > 0) {
    return "BLOCKED";
  }
