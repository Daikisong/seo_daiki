      ...
      <div className="mx-auto grid max-w-[1170px] gap-12 px-4 py-8 xl:min-h-[960px] xl:grid-cols-[minmax(0,700px)_300px] xl:gap-[90px] xl:px-10">
        <div>
          ...
          <section id="latest-posts">
            {archiveTitle ? (
              <div className="mb-6">
                <h1 className="text-[21px] font-bold leading-[23.1px] tracking-normal text-[#2b2f33] md:text-[25.6px] md:leading-[28.16px] xl:text-[32px] xl:leading-[35.2px]">{archiveTitle}</h1>
            ...
            <div>
              {filteredArticles.length > 0 ? (
                ...
                <div>
                  <h2 className="text-[21px] font-bold leading-[23.1px] tracking-normal text-[#2b2f33] md:text-[25.6px] md:leading-[28.16px]">No posts found.</h2>
                </div>
              )}
            </div>
          </section>
        ...
        <aside className="hidden space-y-8 pt-1 xl:block">
          <SidebarSearch actionHref={activeCategory?.href ?? "/"} searchQuery={searchQuery} />
  ...
  return (
    <section className="mb-10 max-w-[740px] text-neutral-900">
      <h1 className="text-[21px] font-bold leading-[23.1px] tracking-normal text-[#2b2f33] md:text-[25.6px] md:leading-[28.16px] xl:text-[32px] xl:leading-[35.2px]">
        {trendSiteName} - Trend-Led Buyer Guides Across Marketplaces
      </h1>
      <div className="mt-[25px] space-y-[25px] text-[15px] leading-[21px] text-neutral-800 md:text-base md:leading-6">
        ...
        <p>
          Welcome to {trendSiteName} - my small buying desk for products that suddenly become hard to judge across
          AliExpress, Temu, Amazon, iHerb, and other fast-moving marketplaces.
        </p>
        <p>
          For most guides, I compare public specs, live listing claims, price snapshots, seller terms, warranty or
          return paths, credible review coverage, and repeated buyer complaints.
        </p>
        <p>
          {trendHomeDescription} The goal is to make the buying decision clearer: why a product is worth checking, who
          it fits, who should skip it, and what can go wrong after checkout.
        </p>
      </div>
    </section>
  );
...
export function filterArchiveArticles(articles: Article[], searchQuery: string, activeCategory?: TrendCategory) {
  ...
  return articles.filter((article) => {
    ...
    return `${article.title} ${article.summary} ${category.label}`.toLowerCase().includes(normalizedQuery);
  });
}
  ...
  return (
    <article className="pb-8">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-8 gap-y-2 text-[12.75px] leading-[21.675px] text-[#90949a] md:mt-4 md:text-[13.6px] md:leading-[23.12px]">
        ...
        <span>
          Category:{" "}
          <Link className="focus-ring rounded-sm text-[#90949a] hover:text-[#2f7cd3]" href={category.href}>
            {category.label}
          </Link>
        </span>
        <span>Author: Jacob</span>
      </div>
      <Link className="focus-ring mt-[14px] block self-start overflow-hidden bg-white md:mt-5" href={articlePath(article)}>
        <Image
          alt={`${article.title} thumbnail`}
      ...
      <p className="mt-[25px] text-[15px] leading-[21px] text-neutral-800 md:text-base md:leading-6">{postExcerpt(article.summary)}</p>
      <Link className="focus-ring mt-[14px] inline-block rounded-sm text-[15px] leading-[21px] text-[#2f7cd3] hover:text-[#1f5f9f] md:text-base md:leading-6" href={articlePath(article)}>
        Read more
      </Link>
    </article>
  );
...
function SidebarSearch({ actionHref, searchQuery }: { actionHref: string; searchQuery: string }) {
  return (
    <section>
...
function SidebarSection({ children, title }: { children: ReactNode; title: string }) {
  return (
    <section>
      <h2 className="text-[24px] font-bold leading-[26.4px] tracking-normal text-[#2b2f33]">{title}</h2>
