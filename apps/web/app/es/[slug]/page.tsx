...
export async function generateStaticParams() {
  ...
  return params.filter((param) => param.locale === "es").map((param) => ({ slug: param.slug }));
}

export async function generateMetadata({ params, searchParams }: PageProps) {
  const { slug } = await params;
  return generateArticleMetadata(Promise.resolve({ locale: "es", slug }), "hub", searchParams);
}

export default async function HubPage({ params, searchParams }: PageProps) {
  const { slug } = await params;
  const page = await loadArticlePage(Promise.resolve({ locale: "es", slug }), "hub", searchParams);
