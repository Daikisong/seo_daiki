import { ArticlePage } from "@/components/layout/ArticlePage";
import {
  generateCountryRiskGuideMetadata,
  loadCountryRiskGuidePage,
  staticCountryRiskGuideParamsFor
} from "@/lib/content/page-loaders";

export const dynamic = "force-dynamic";

interface PageProps {
  params: Promise<{ slug: string }>;
  searchParams?: Promise<{ previewToken?: string | string[] }>;
}

export function generateStaticParams() {
  return staticCountryRiskGuideParamsFor("en-us", "guides");
}

export function generateMetadata({ params, searchParams }: PageProps) {
  return generateCountryRiskGuideMetadata(params, "en-us", "guides", searchParams);
}

export default async function UsCountryRiskGuidePage({ params, searchParams }: PageProps) {
  const page = await loadCountryRiskGuidePage(params, "en-us", "guides", searchParams);
  return <ArticlePage {...page} />;
}
