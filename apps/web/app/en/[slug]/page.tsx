...
export async function generateStaticParams() {
  ...
  return params.filter((param) => param.locale === "en").map((param) => ({ slug: param.slug }));
}

export async function generateMetadata({ params, searchParams }: PageProps) {
  const { slug } = await params;
  return generateArticleMetadata(Promise.resolve({ locale: "en", slug }), "hub", searchParams);
}

export default async function HubPage({ params, searchParams }: PageProps) {
  const { slug } = await params;
  const page = await loadArticlePage(Promise.resolve({ locale: "en", slug }), "hub", searchParams);
