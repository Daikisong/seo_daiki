import type { Article, ArticleSection, InternalLink, Locale } from "@global-import-lab/types";
import type { ArticleDraft } from "./article-draft-types";

interface BuildBaseDraftArticlesOptions {
  updatedAt: string;
  internalLinks: (locale: Locale) => InternalLink[];
  sections: (headings: string[], evidenceIds: string[]) => ArticleSection[];
}

export function buildBaseDraftArticles({
  updatedAt,
  internalLinks: sampleInternalLinks,
  sections: sampleSections
}: BuildBaseDraftArticlesOptions): ArticleDraft[] {
